  ...
  return (
    <div className="bg-[var(--card)] border border-[var(--border)] rounded-lg p-4 mb-6">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-sm text-gray-400">{completed}/3 complete</span>
      </div>
          ...
          <div key={step.label} className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${
                step.done ? "border-green-500 bg-green-500" : "border-gray-600"
              }`}>
                {step.done && (
              ...
              <span className={`text-sm ${step.done ? "text-gray-500 line-through" : ""}`}>
                {step.label}
              </span>
            </div>
            {!step.done && (
              <Link href={step.href} className="text-xs text-[var(--primary)] hover:underline">
                {step.action}
              </Link>
            )}
          </div>
